...
void APatK(const std::map<unsigned, T> &real_map, const std::map<unsigned, T> &query_map, double &ap, double *p = NULL, double *r = NULL)
{
	...
	unsigned hit_sum = 0;
	for (std::map<unsigned, T>::const_iterator iter_query = query_map.cbegin(); iter_query != query_map.cend(); ++iter_query)
	{
		for (std::map<unsigned, T>::const_iterator iter_real = real_map.cbegin(); iter_real != real_map.cend(); ++iter_real)
		{
	...
	ap = 0;
	unsigned hit = 0;
	for (std::map<unsigned, unsigned>::const_iterator iter_ap = ap_map.cbegin(); iter_ap != ap_map.cend(); ++iter_ap)
	{
		if (iter_ap->second == 1)
		{
			++hit;
