...
static std::ofstream algo_out_file("D:\\Experiment\\M2LS\\datasets\\algo_testing.txt", std::ofstream::out | std::ofstream::trunc);

void print_graph(const typed_directed_graph &graph)
{
	std::ofstream out_file("D:\\Experiment\\M2LS\\datasets\\graph.txt", std::ofstream::out | std::ofstream::trunc);
	if (!out_file.fail())
	{
		out_file << "vs ve dist\n---------------\n";
		int edges_count = 0;
	...
	virtual bool operator()(const target_vertex &v, const definite_hash_map &hash_map)
	{
		if (v.type == 'f') // nearest facility is found
		{
			...
			for (; iter_edge != ptr_loc_edges_hash_map->end(); ++iter_edge)
			{
				...
				std::string vs_ve = iter_edge->first; // the edge vs->ve, "vs_id-ve_id"
...
public:
	...
	{
		std::stringstream ss_vs_ve; // the edge (id) to be traversed, format must be "vs_id-ve_id"

		if (v.type != 'r') // impossible to be 'f', then must be 'v'
		{
			if (e.ve.type != 'f') // must be 'v' (e.dist is impossible to be zero), then v->e.ve
			{
				ss_vs_ve << v.id << "-" << e.ve.id;
				(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, v.dist, v.dist + e.dist, e.ve.id);
#ifdef ALGO_TESTING
			...
			else // e.ve.type == 'f'
			{
				...
				else // e.dist != 0, e.ve is a facility that must split an edge
				{
					...
					{
						ss_vs_ve << v.id << "-" << iter_fac->second.get<VE_ID>();
						(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, v.dist, v.dist + iter_fac->second.get<VS_DIST>() + iter_fac->second.get<VE_DIST>(),
							iter_fac->second.get<VE_ID>());
#ifdef ALGO_TESTING
					...
					{
						ss_vs_ve << v.id << "-" << iter_fac->second.get<VS_ID>();
						(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, v.dist, v.dist + iter_fac->second.get<VE_DIST>() + iter_fac->second.get<VS_DIST>(),
							iter_fac->second.get<VS_ID>());
#ifdef ALGO_TESTING
		...
		else // v.type == 'r'
		{
			...
			else // e.dist != 0, v(r) must split an edge
			{
				if (e.ve.type != 'f') // e.ve is ordinary vertex
				{
					...
					{
						ss_vs_ve << vs_id << "-" << ve_id;
						(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, 0.0f, e.dist, ve_id);
#ifdef ALGO_TESTING
					...
					{
						ss_vs_ve << ve_id << "-" << vs_id;
						(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, 0.0f, e.dist, vs_id);
#ifdef ALGO_TESTING
						algo_out_file << ss_vs_ve.str() << ", d(r,vs) = " << 0 << ", d(r,vs)+d(vs,ve) = " << e.dist << '\n';
#endif
						return false; // a flag to continue graph traversal
				...
				else // e.ve.type == 'f', r and f locate on and split the same edge
				{
					...
					if (iter_fac->second.get<VS_ID>() == vs_id)
					{
						...
						{
							ss_vs_ve << ve_id << "-" << vs_id;
							(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, 0.0f, dist_vs, vs_id);
#ifdef ALGO_TESTING
						...
						{
							ss_vs_ve << vs_id << "-" << ve_id;
							(*ptr_loc_edges_hash_map)[ss_vs_ve.str()] = boost::make_tuple(prob, 0.0f, dist_ve, ve_id);
#ifdef ALGO_TESTING
	...
	if (!ifs_cands.fail()) // candidates file exists
	{
		while (!ifs_cands.bad() && ifs_cands.good())
		{
			...
			if (vs_id != ve_id) // candidate splits an edge
			{
				...
				std::stringstream ss_vs_ve, ss_ve_vs;
				ss_vs_ve << vs_id << "-" << ve_id; // vs->ve, format must be "vs_id-ve_id"
				ss_ve_vs << ve_id << "-" << vs_id; // ve->vs, reverse direction

				// retrieve ERD+ of edge
				float ERD_u = 0.0f; // ERD+ for ordering candidates in max-heap
				LNT_hash_map::const_iterator iter_vs_ve = LNT.find(ss_vs_ve.str()); // vs->ve
				LNT_hash_map::const_iterator iter_ve_vs = LNT.find(ss_ve_vs.str()); // ve->vs
				 ...
				 //		  5) in the special case, where c overlaps v and some r(s) also overlap v, we compute ERD with the help of vertex_nnd_hash_map
			{
				...
				for (; iter_out_edge != iter_vertex->second.first.cend(); ++iter_out_edge) // compute ERd for each out-edge
				{
					...
					ss_vs_ve << vs_id << "-" << out_v_id; // vs->ve, format must be "vs_id-ve_id"
					LNT_hash_map::const_iterator iter_vs_ve = LNT.find(ss_vs_ve.str()); // vs->ve
